...
function get_seo_route($slug) {
    ...
    // Aliases & canonical consolidations for variants
    $aliases = [
        'gasfiter-sec-2' => 'gasfiter-certificado-sec',
        'gasfiter-sec-chile' => 'gasfiter-sec-chile',
        'gasfiter-certificado-2' => 'gasfiter-certificado',
    ...
    if (isset($aliases[$clean_slug]) && isset($routes[$aliases[$clean_slug]])) {
        return $routes[$aliases[$clean_slug]];
    }
    
    // Dynamic fallback for any other gasfiter-* or comuna slug
    if (str_starts_with($clean_slug, 'gasfiter-') || str_starts_with($clean_slug, 'gasfiter')) {
        $human_title = ucwords(str_replace(['gasfiter-', 'certificado-', '-'], ['Gásfiter ', 'Certificado ', ' '], $clean_slug));
        return [
